...
class Balancer:
    ...
    def __init__(self, name:str='balance',msg=True):
        ...
        self._n_retry = 3
        self._n_worker = 10
        self._s_restart_remain_done = 10
        self._h_restart_remain_started = 1*60*60

    def set_config(self, n_worker=None, n_retry=None,s_restart_remain_done=None,h_restart_remain_started=None):
        if n_worker is not None: self._n_worker = n_worker
        if n_retry is not None: self._n_worker = n_retry
        if s_restart_remain_done is not None: self._s_restart_remain_done = s_restart_remain_done
        if h_restart_remain_started is not None: self._h_restart_remain_started = h_restart_remain_started*60*60
        ...
        while True:
            ...
            if item is None:
                ...
                try:
                    ...
                    if item.retry < 3:
                        await self._xput_queue(
                            name=item.name, args=item.args, kwargs=item.kwargs, retry=item.retry + 1)
                        self._custom.warning.msg('exception',f'retry({item.retry+ 1})', item.name, str(item.args))
                    else:
                        result = None
                        self._custom.error.msg('exception',f'retry({item.retry+ 1})', item.name, str(item.args))
    ...
    async def server(self,n_worker:int=None,msg_run=False,msg_done=False):
        n_worker = self.n_worker if n_worker is None else n_worker
            ...
            while True:
                ...
                if tasks_started and self._queue._unfinished_tasks == 0:
                    ...
                    while self._queue._unfinished_tasks == 0:
                        ...
                        is_restart_done = loop.time() - time_tasks_done >= self._s_restart_remain_done
                        is_restart_started = loop.time() - time_tasks_started >= self._h_restart_remain_started
...
if __name__ == "__main__":
    ...
    balance.set_config(h_restart_remain_started=0)
    # async def client():
    #     aa = await balance.xfetch('asleep', (2,))
    #     print(aa)
    #     bb = await  balance.xfetch('asleep', (2,))
    #     print(bb)
    #     cc = await  balance.xfetch('asleep', (3,))
    #     print(cc)

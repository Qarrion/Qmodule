...
class CandleItem(pg.GraphicsObject):
    ...
    def set_tohlc(self, data):
        """[datetime, open, high, low, close]"""
        self._tohlc = self._tohlc(data) 
        self.gen_picture()
        self.update() # 변경 사항을 적용하고, 재그리기 요청

    def _tohlc(self, ohlc:list|tuple):
        arr = np.array(ohlc)
        if arr.ndim == 1:
    ...
    def gen_picture(self):
        ...
        if self.data.size > 0 :
            for (index, open, high, low, close ) in self.data:
                if open > close:
                    p.setPen(pg.mkPen('#4584b6'))
                    p.setBrush(pg.mkBrush('#4584b6'))
                else:
                    p.setPen(pg.mkPen('#ffde57'))
                    p.setBrush(pg.mkBrush('#ffde57'))

...
class AxisItem(pg.AxisItem):
    ...
    def drawPicture(self, p, axisSpec, tickSpecs, textSpecs):
        ...
        #     if text==self.y_target :  # 여기에 특정 조건을 추가
...
class  PlotWidget(pg.PlotWidget):
    ...
    def mouseMoveEvent(self, event):
        ...
        print(self.getPlotItem().getAxis('left')._values)
        for lev in self.axis_l._values:

            ticks.append([(v, f"{v:.2f}") for v in lev[1]])
        # ticks.append([(y_value, f"{y_value:.2f}")])
            y_target = f"{y_value:.2f}"
            self.axis_l.set_target(y_target)
        ticks[-1].append((y_value, y_target))
        # print(ticks)
        # self.plot_item.getAxis('left').setTicks(ticks)
        # self.plot_item.getAxis('left')5

...
from Qutils.custom_print import dprint
...
class Config:
    ...
    def get(self, section, option, *, raw: bool = False, vars= None):
        if self.parser.has_section(section):
            dprint(f"section ('{section}')",1,self._msg)
            return self.parser.get(section, option, raw=raw, vars=vars)
        
        elif self.parser.has_section('default'):
            dprint(f"section ('{section}')",0,self._msg)
            dprint(f"section ('{'defualt'}')",1,self._msg)
            return self.parser.get('default', option, raw=raw, vars=vars)
        else:
